<div class="container">
  ...
  <div style="display:flex; gap:8px; align-items:center; margin-top:8px;">
    ...
    <input id="author" value="webpick">
    <input id="agency" value="MYNET">
...
async function loadTrace(){
  ...
  if(!res.ok){ document.getElementById('log').textContent = await res.text(); return; }
  ...
  Plotly.newPlot("plot", [{x:xAbs,y:data.y,mode:"lines",name:`${currentMeta.station}.${currentMeta.channel}`}],
    {margin:{l:40,r:10,t:20,b:40}, xaxis:{title:"Tempo (UTC)"}, yaxis:{title:"Counts"}});

  const plot = document.getElementById("plot");
  plot.on("plotly_click", (ev)=>{
    const t = ev.points[0].x; // Date
...
async function sendPick(){
  ...
  const payload = {
    ...
    network: currentMeta.network, station: currentMeta.station,
    location: currentMeta.location, channel: currentMeta.channel,
